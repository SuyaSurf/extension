/**
 * Universal Content Script Handler (TypeScript)
...
console.log('Suya Bot Content Script (TypeScript) initialized');
...
function detectPageFeatures(): any {
  return {
...
function detectResponsiveDesign(): boolean {
  const hasViewportMeta = !!document.querySelector('meta[name="viewport"]');
  return hasViewportMeta; // Simplified check
}

function scanPageElements(): any {
  return {
    headings: Array.from(document.querySelectorAll('h1, h2, h3, h4, h5, h6')).map((el: any) => ({
      tag: el.tagName,
      text: el.textContent?.trim(),
      level: parseInt(el.tagName.substring(1))
    })),
    paragraphs: Array.from(document.querySelectorAll('p')).map((el: any) => ({
      text: el.textContent?.trim(),
      length: el.textContent?.length || 0
    })),
    links: Array.from(document.querySelectorAll('a[href]')).map((el: any) => ({
      href: el.href,
      text: el.textContent?.trim(),
      isExternal: el.hostname !== window.location.hostname
    })),
    buttons: Array.from(document.querySelectorAll('button, input[type="button"], input[type="submit"]')).map((el: any) => ({
      text: el.textContent?.trim() || el.value,
      type: el.type,
      disabled: el.disabled
    })),
    inputs: Array.from(document.querySelectorAll('input, textarea, select')).map((el: any) => ({
      type: el.type,
      name: el.name,
...
function scanForms(): any[] {
  return Array.from(document.querySelectorAll('form')).map((form: any) => ({
    action: form.action,
    method: form.method,
    id: form.id,
...
function scanMediaElements(): any {
  return {
    videos: Array.from(document.querySelectorAll('video')).map((video: any) => ({
      src: video.src,
    ...
    audios: Array.from(document.querySelectorAll('audio')).map((audio: any) => ({
      src: audio.src,
    ...
    images: Array.from(document.querySelectorAll('img')).map((img: any) => ({
      src: img.src,
      alt: img.alt,
      width: img.width,
      height: img.height
...
function handleWindowMessage(event: MessageEvent) {
  ...
  if (message.type === 'suya-command') {
    handleCommand(message.data?.command, message.data?.data);
  }
}
...
function executeSkillAction(data: any) {
...
function injectUIElements(customData?: any) {
  // Create Suya bot
...
function positionSuyaBot() {
  ...
  (bot as HTMLElement).style.left = x + 'px';
  (bot as HTMLElement).style.top = y + 'px';
  (bot as HTMLElement).style.position = 'fixed';
  (bot as HTMLElement).style.zIndex = '9999';
}
...
function removeUIElements(elements?: any) {
